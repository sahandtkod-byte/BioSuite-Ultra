...
from biosuite.api.security import verify_admin_token, create_access_token, get_admin_credentials, hash_password, verify_password
...
MAX_REQUEST_SIZE_MB = int(os.environ.get("BIOSUITE_MAX_REQUEST_SIZE_MB", "50"))
MAX_SEQUENCE_LENGTH = int(os.environ.get("BIOSUITE_MAX_SEQUENCE_LENGTH", "10000000"))
...
async def limit_request_size(request, call_next):
    ...
    if content_length and int(content_length) > MAX_REQUEST_SIZE_MB * 1_048_576:
...
class SequenceRequest(BaseModel):
    sequence: str = Field(..., description="DNA or protein sequence", min_length=1, max_length=MAX_SEQUENCE_LENGTH)
...
class TranslationRequest(BaseModel):
    sequence: str = Field(..., description="DNA sequence to translate", max_length=MAX_SEQUENCE_LENGTH)
...
async def admin_login(username: str, password: str):
    ...
    if username != admin_user or not verify_password(password, admin_pass):
