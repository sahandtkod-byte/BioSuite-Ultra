...
def kallisto_quant(reads_r1, reads_r2=None, index_file=None,
    ...
    if index_file is None and transcriptome_fasta:
        _fd, _idx_path = tempfile.mkstemp(suffix='.idx'); os.close(_fd)
...
def quantify_reads(reads_file, transcriptome_fasta, sample_name='sample', k=31):
    ...
    if tools['kallisto']:
        _fd, _idx_tmp = tempfile.mkstemp(suffix='.idx'); os.close(_fd)
        idx = kallisto_index(transcriptome_fasta, _idx_tmp)
        if idx:
            out = tempfile.mkdtemp()
