...
class _DictWrapper(dict):
    """Dict subclass that persists changes and supports attribute access.
    
    Backward-compatible with code that does config.get(), config['key'], etc.
    """
    def __init__(self, data=None, save_fn=None):
...
def save_session(session_data):
    try:
        with open(SESSION_FILE, 'w') as f:
            json.dump(dict(session_data), f, indent=2)
    except OSError:
        pass
...
config = _DictWrapper(load_config(), save_fn=save_config)
session = _DictWrapper(load_session(), save_fn=save_session)
