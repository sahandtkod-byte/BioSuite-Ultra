...
# ── Streaming Readers (memory-efficient for large files) ──────────────

def iter_fasta(filepath: str):
    """Yield (header, sequence) tuples from a FASTA file without loading all into memory.
    
    Memory-efficient for files with millions of sequences. Each sequence
    is yielded as a complete string, but only one is in memory at a time.
...
def iter_fastq(filepath: str):
    """Yield (name, sequence, quality) tuples from a FASTQ file.
    
    Memory-efficient streaming parser — one record at a time.
    
    Args:
        filepath: Path to a .fastq or .fq file.
    
    Yields:
        (name, sequence, quality) tuples.
    """
    if not os.path.exists(filepath):
        return
    try:
