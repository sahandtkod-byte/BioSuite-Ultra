...
### 🔒 Security (BREAKING)
- **All hardcoded credentials removed** — must set `BIOSUITE_API_KEY`, `BIOSUITE_ADMIN_USER`, `BIOSUITE_ADMIN_PASSWORD` env vars
- **Password hashing** via PBKDF2-SHA256 (no plaintext storage)
- **Secure temp files** — replaced 15 insecure `tempfile.mktemp()` calls

### ⚡ Performance
- **32x faster imports** — core module load time: 10.65s → 0.33s via lazy loading

### 🚀 New Features
- Streaming FASTA/FASTQ readers for large files
- Jupyter notebook magics + widgets as separate modules
- Structured logging with rotation
- Request size limits + sequence length validation
- Docker support (API + GUI)
- GitHub Actions CI/CD
- 63 tests (parametrized)
